...
function Invoke-WebClone {
    ...
    $CreatedPortGroups = New-PodPortGroups -Portgroups $Pods -StartPort $FirstPodNumber -EndPort ($FirstPodNumber + $Pods + 50) -Tag $Tag -AssignPortGroups $AssignPortGroups

    New-VApp -Name ( -join ($CreatedPortGroups[0], '_Pod')) -Location (Get-ResourcePool -Name $Target -ErrorAction Stop) -ErrorAction Stop | New-TagAssignment -Tag $Tag | Out-Null
    ...
    if ($CreateRouters -eq $true) {
        ...
        if ($CompetitionSetup -eq $True) {
            New-PodRouter -Target $SourceResourcePool -WanPortGroup $WanPortGroup -PFSenseTemplate '1:1NAT_PodRouter' -ErrorAction Stop | Out-Null
                ...
                New-PodRouter -Target ( -join ($CreatedPortGroups[0], '_Pod')) -WanPortGroup $WanPortGroup -LanPortGroup ( -join ($CreatedPortGroups[0], '_PodNetwork')) -PFSenseTemplate 'pfSense Blank' | Out-Null
        }
    }
    ...
    $Tasks = foreach ($VM in $VMsToClone) {
        Get-VApp -Name ( -join ($CreatedPortGroups[0], '_Pod')) | ForEach-Object { New-VM -VM $VM -Name ( -join (($_.Name.Split("P")[0]), $VM.Name)) -ResourcePool (Get-VApp -Name ($_.Name)).Name -LinkedClone -ReferenceSnapshot "SnapshotForCloning" -RunAsync }
    }
    ...
    if ($AssignPortGroups) {
        #Set Variables
        $Routers = Get-VApp -Name ( -join ($CreatedPortGroups[0], '_Pod')) -ErrorAction Stop | Get-VM | Where-Object -Property Name -Like '*PodRouter*'
        $VMs = Get-VApp -Name ( -join ($CreatedPortGroups[0], '_Pod')) -ErrorAction Stop | Get-VM | Where-Object -Property Name -NotLike '*PodRouter*'
        ...
        $Routers | 
            ...
            $tasks = Get-VApp -Name ( -join ($CreatedPortGroups[0], '_Pod')) | Get-VM -Name *PodRouter | Start-VM -RunAsync

            Wait-Task -Task $tasks -ErrorAction Stop

            Start-Sleep 30

            Get-VApp -Name ( -join ($CreatedPortGroups[0], '_Pod')) | 
                Get-VM -Name *PodRouter |
                    Select -ExpandProperty name | 
                        ForEach-Object { 
                            $oct = $_.split("_")[0].substring(2)
    ...
    #Snapshot new VMs
    Get-VApp -Name ( -join ($CreatedPortGroups[0], '_Pod')) | Get-VM | ForEach-Object { New-Snapshot -VM $_ -Name 'Base' -Confirm:$false -RunAsync }
    ...
    if ($CreateUsers -eq $true) {
        ...
        Write-Host 'Creating the pod users...'
        $podcred = New-PodUsers -Pods $names -Role $Role -Description $Tag -Domain $Domain | Out-Null
    }
